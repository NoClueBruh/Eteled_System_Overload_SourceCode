...
import NoClueCloset.Popup_Settings;
import NoClueCloset.Popup;
...
class PlayState extends MusicBeatState
{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'spookeez' | 'south' | 'monster':
					curStage = 'spooky';
				case 'dream_of_peace' | 'diagraphephobia' | 'post_mortal' | 'dream_of_peace' | 'system_faliure' | 'plaything' | 'dream_of_peace' | 'modernized' | 'distress' | 'bananaled':
					gf.visible = false;
	...
	override public function update(elapsed:Float)
	{
		...
		NoClueCloset.Popup.u_p_d_a_t_e(elapsed);

		FlxG.mouse.visible = FlxG.mouse.useSystemCursor = Popup.popupCount > 0;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			if(Popup.popupSongs.exists(SONG.song.toLowerCase()))
			{
				cld--;

				var current:Popup_Settings = Popup.popupSongs[SONG.song.toLowerCase()];
				if(cld < 0 &&FlxG.random.bool(current.interval))
				{
					for(_ in 0...Std.int(Math.max(1, FlxG.random.int(-1, 3))))
					{
						if(mpn+ 1 < current.maxPopups){
							add(new Popup());
							mpn++;
						}
					}

					cld = current.cooldown;
				}
				
				n++;
				if(n > current.maxPopups_n){
					n = 0;
					mpn = 0;
				}
			}
